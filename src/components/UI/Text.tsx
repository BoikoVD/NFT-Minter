...
export default function Text({ children, className = '', leading = 'loose', size = 'small', color = 'white' }: IText) {
    
    return <p className={`${styles[leading]} ${styles[size]} ${styles[color]} ${className}`}>{children}</p>;
};